...
#define TEXT_SZ 10000  //same as value in MAX_AUDIT_MESSAGE_LENGTH in auditd
#define SHARED_SIZE 1000

//缓冲池结构
struct shared_use_st  
{  
    int Index[SHARED_SIZE];    //SHARED_SIZE个缓冲池,为0表示对应的缓冲区未被生产者使用，可分配但不可消费；为1表示对应的缓冲区已被生产者使用，不可分配但可消费
    char Buffer[SHARED_SIZE][TEXT_SZ];    //SHARED_SIZE个字符串缓冲区
    sem_t sem;        //信号量，同步功能
};

int shared_memory_init = 0;
...
int init_shared_memory(){
    shmid = shmget((key_t)4343, sizeof(struct shared_use_st), 0666|IPC_CREAT);
	...
	//bv_loge("Init the shared memory[%dKB] failed [%d:%s]!",size/1024, errno, strerror(errno));
      ...
      //fprintf(log_file, "%d %s %d:%s\n",  sizeof(struct shared_use_st)/1024,"KB",errno,strerror(errno));
      return -1;
    }
...
static void write_to_log(const struct auditd_event *e)
{
	...
	{
		int status = init_shared_memory();
		//if(status){
			//fprintf(log_file, "%s\n", "Shared memory init failed");
			//return; //memory init failed
		//}
	}
	...
	if(sem_wait(&(shared->sem)) == -1)        //sem_wait为P操作，减少信号量的值
    	{
            //printf("P操作 ERROR!\n");
			fprintf(log_file, "%s\n", "Error in P operation ");
            return;
    	}
	

	produce_index = produce_index % SHARED_SIZE;
	if(shared->Index[produce_index] == 1)
	{
            //which should be written in but not empty 
            sem_post(&shared->sem);        //sem_post为V操作，用来增加信号量的值
	    //fprintf(log_file, "%s\n", "Procude waiting for some time...");
    	}
    	else
	{
	    int length = strlen(e->reply.message);
			//读取的字符串存入缓冲区shared->Buffer中
            strncpy(shared->Buffer[produce_index],e->reply.message,length);
	    shared->Buffer[produce_index][length] = '\n';
	    shared->Buffer[produce_index][length+1]='\0';
            shared->Index[produce_index] = 1;        //表示该缓冲区被生产者使用了
            sem_post(&shared->sem);        //V 操作增加信号量
	    //fprintf(log_file, "%s", e->reply.message);
	    produce_index++;
    	}
